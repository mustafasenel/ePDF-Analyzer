...
import { Upload, FileText, AlertCircle, Loader2, Table2 } from 'lucide-react'
...
interface BasicExtractionResult {
  status: string
  filename: string
  page_count: number
...
export default function BasicExtraction() {
  const [file, setFile] = useState<File | null>(null)
  const [pdfUrl, setPdfUrl] = useState<string | null>(null)
  const [loading, setLoading] = useState(false)
  const [result, setResult] = useState<BasicExtractionResult | null>(null)
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const response = await fetch('/api/v1/extract/basic', {
        method: 'POST',
        body: formData,
      })
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold">Basic Extraction</h1>
        <p className="text-muted-foreground mt-1">
          PDF'den sayfa bazında text ve tablo çıkarımı (yorumlama yok)
        </p>
      </div>
      ...
      {/* Results Section: PDF Viewer + Data */}
      {result && pdfUrl && (
                  ...
                  <Tabs defaultValue="text">
                    <TabsList className="grid w-full grid-cols-3">
                      ...
                      <TabsTrigger value="tables">
                        ...
                        Tables ({result.pages[selectedPage - 1].table_count})
                      </TabsTrigger>
                    ...
                    <TabsContent value="text" className="mt-4">
                      <ScrollArea className="h-[400px] w-full rounded-md border p-4">
                        <pre className="text-xs whitespace-pre-wrap font-mono">
                          {result.pages[selectedPage - 1].text || 'Metin bulunamadı'}
                        </pre>
                      </ScrollArea>
                      <p className="text-xs text-muted-foreground mt-2">
                        {result.pages[selectedPage - 1].text_length} karakter
                      </p>
                    </TabsContent>

                    <TabsContent value="tables" className="mt-4">
                      <ScrollArea className="h-[400px] w-full">
                        {result.pages[selectedPage - 1].tables.length > 0 ? (
                          <div className="space-y-4">
                            {result.pages[selectedPage - 1].tables.map((table: any, idx: number) => (
                    ...
                    <TabsContent value="json" className="mt-4">
                      <ScrollArea className="h-[400px] w-full">
                        <JsonHighlight
                          json={JSON.stringify(result.pages[selectedPage - 1], null, 2)}
